...
async function runAggregation(mode = 'github', pages = 50) {
    ...
    try {
        ...
        await new Promise(r => setImmediate(r)); // 让出主线程
        let validProxies = await validateProxies(proxies, 50, 10000);
...
async function runConnectivityCheck() {
    ...
    // 自动移除超时节点，只保留可连通的
    const beforeRemove = proxies.length;
    const aliveProxies = proxies.filter(p => p.localLatency !== -1);
    const removedCount = beforeRemove - aliveProxies.length;

    if (removedCount > 0) {
        addLog(`🗑️ 自动移除 ${removedCount} 个超时节点`, 'info');
    }

    // 保存可用节点（超时的不再保留）
    fs.writeFileSync(proxiesFile, JSON.stringify(aliveProxies, null, 2));
    addLog(`✅ 连通性检测完成: ${passed}/${beforeRemove} 个可用，已移除 ${removedCount} 个超时节点`, 'success');

    return { tested: beforeRemove, passed, failed, removed: removedCount };
}
