...
// 导入节点的通用函数
async function importNodes(url, btnId, sourceName) {
    const importBtn = document.getElementById(btnId);
    const originalText = importBtn.textContent;

    console.log(`开始导入${sourceName}配置...`);

    try {
        importBtn.disabled = true;
        importBtn.textContent = '正在导入...';
        showLoading(`正在从${sourceName}下载配置...`);
...
// 导入全域扫描节点 (原机场聚合器)
function importGlobalNodes() {
    importNodes(
        'https://us.liukun.com:8443/Projects/Aggregator/Aggregator.yaml',
        'importGlobalBtn',
        '全域扫描节点'
    );
}
...
// 导入本地扫描节点 (新增)
function importLocalNodes() {
    importNodes(
        'https://home.liukun.com:8443/Projects/Aggregator/Aggregator.yaml',
        'importLocalBtn',
        '本地扫描节点'
    );
}
...
async function handleFileContent(file, yamlContent) {
    try {
        ...
        // 改为发送 Raw Body，避免 FormData 带来的额外开销和潜在截断问题
        const response = await fetch(`api/parse.php?filename=${encodeURIComponent(file.name)}`, {
            method: 'POST',
            headers: {
        ...
        if (result.success) {
            ...
            if (result.debug_total_found) {
                console.log(`Debug: YAML Parser found ${result.debug_total_found} proxies`);
            }
            ...
            // 2. 不再进行前端去重，依靠后端根据UUID/密码去重
            // 防止同服务器不同账号被误删
            const newNodes = filteredNodes;
            const removedByDedupe = 0;

            console.log(`节点处理: 原始 ${originalCount} 个 -> 过滤 ${filteredCount} 个广告节点 -> 剩余 ${newNodes.length} 个`);

            if (filteredCount > 0) {
                if (window.CyberpunkAnimations) {
                    CyberpunkAnimations.showNotification(
                        `已过滤 ${filteredCount} 个广告节点`,
                        'info'
                    );
                }
            }
            ...
            if (mergeResult.success) {
                ...
                if (window.CyberpunkAnimations) {
                    CyberpunkAnimations.showNotification(`✓ 导入成功: 获取 ${stats.total} 个 / 新增 ${stats.added} 个 / 更新 ${stats.updated} 个`, 'success');
                }
                ...
                if (window.CyberpunkAnimations) {
                    CyberpunkAnimations.showNotification('导入完成。请点击"开始检测"按钮。', 'info');
                }

                // 提示用户手动开始检测
                const btn = document.getElementById('startCheckBtn');
                if (btn) {
                    btn.classList.add('pulse-once'); // 如果有这个类
...
function showStats() {
    ...
    // 直接显示数值，避免动画可能导致的显示延迟或错误
    if (window.CyberpunkAnimations && false) { // 暂时禁用动画
...
function deduplicateNodesByServer(nodeList) {
    ...
    nodeList.forEach(node => {
        ...
        if (!server || !port) {
            // 没有服务器地址或端口的节点也保留(防止误删)
            deduplicated.push(node);
            return;
        }

        // 使用 server:port 作为唯一标识
