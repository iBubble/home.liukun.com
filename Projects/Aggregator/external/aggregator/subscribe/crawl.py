...
def _parse_yaml_proxies(content: str):
...
def collect_airport(
    ...
    def crawl_jctj(convert: bool = False) -> dict:
        ...
        # 使用requests库代替utils.http_get，因为urllib在访问GitHub时可能失败
        try:
            import requests
            response = requests.get(url, timeout=30)
            if response.status_code != 200:
                logger.error(f"[AirPortCollector] failed to fetch from [{url}], status: {response.status_code}")
                return {}
            content = response.text
        except Exception as e:
            logger.error(f"[AirPortCollector] failed to fetch from [{url}], error: {e}")
            # 降级到utils.http_get
            content = utils.http_get(url=url)
            if not content:
                return {}
