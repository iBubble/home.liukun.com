...
async function testWithoutTFO() {
    ...
    const configPath = path.join(CLASH_DIR, 'test_no_tfo_config.yaml');
    fs.writeFileSync(configPath, yaml.dump(config));
    
    console.log('✅ 配置已生成（TFO已禁用）\n');
    ...
    try {
        const result = execSync(
            `curl -s -x http://127.0.0.1:${TEST_PORT} -k --connect-timeout 15 --max-time 30 -o /dev/null -w "%{http_code}" https://linux.do`,
    ...
    try {
        const result = execSync(
            `curl -s -x http://127.0.0.1:${TEST_PORT} -k --connect-timeout 15 --max-time 30 -o /dev/null -w "%{http_code}" https://www.google.com`,
            { encoding: 'utf8', timeout: 35000 }
        );
        console.log(`  状态码: ${result.trim()}`);
        if (result.trim() === '200') {
...
testWithoutTFO().catch(console.error);
